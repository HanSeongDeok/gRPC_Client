...
public class GrpcClientService2 {
    ...
    public void streamingService(){
        ...
        FitTcInfo tcInfo1 = FitTcInfo.newBuilder().setTcName("TC-001").build();
        HelloRequest.Builder requestBuilder = HelloRequest.newBuilder();
        requestBuilder.addTcInfo(tcInfo1);
        ...
        StreamObserver<HelloRequest> requestStreamObserver = this.stub.streamFromClientToServer(new StreamObserver<HelloReply>() {
            ...
            public void onNext(HelloReply reply) {
                // 서버에서 응답한 데이터 처리
                reply.getInjectionInfoList().stream().forEach(v -> System.out.println(v.getAddress()+"에 " +v.getValue()+"값을 입력 -> 제어기 실행: 실행 내용" ));
                FitRunContent runContent = FitRunContent.newBuilder().setRunContent("실행 내용").build();
                requestBuilder.addRunContent(runContent);
